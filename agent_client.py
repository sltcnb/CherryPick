...
# FH generates gRPC stubs into grpc_stubs/ (not flat). Put it on sys.path so the
# lazy `import collector_pb2` / `import collector_pb2_grpc` below resolve, and so
# collector_pb2_grpc's own bare `import collector_pb2` works.
_STUBS = _os.path.join(_os.path.dirname(__file__), "grpc_stubs")
